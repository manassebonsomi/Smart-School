...
export default class SessionController {
  ...
  async login({ request, response, view, auth }: HttpContext) {
    ...
    try {
      ...
      await auth.use('web').login(user, request.input('remember') === 'on' || request.input('remember') === true)
      if (user.systemRole === 'SUPER_ADMIN') return response.redirect('/super-admin/dashboard')
